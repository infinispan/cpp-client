...
class TestTransportFactory: public TransportObjectFactory {
public:
	TestTransportFactory() :
			TransportObjectFactory(*(protocol::Codec*) NULL,
...
Configuration createConfiguration(int minIdle, int maxActive, int maxTotal) {
	...
	builder.connectionPool().minIdle(minIdle).maxActive(maxActive).maxTotal(
			maxTotal);
...
HR_EXPORT void testMaxActive() {
	...
	BorrowTransportRunnable* runnable = new BorrowTransportRunnable(pool,
			addrs[0]);
...
HR_EXPORT void testMaxTotal2() {
	...
	assert((pool->getNumIdle(*addrs[1]) == (minIdle - 1) && pool->getNumIdle(*addrs[2]) == minIdle) ||
	...
	assert((pool->getNumIdle(*addrs[1]) == (minIdle - 2) && pool->getNumIdle(*addrs[2]) == (minIdle - 1)) ||
